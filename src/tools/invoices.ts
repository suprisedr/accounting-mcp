...
export const tools: Tool[] = [
  {
    name: "invoices",
    description:
      "Manage invoices. Use the 'action' field to select the operation. Requires prior login.\n" +
...
export async function handle(
  ...
  if (name !== "invoices") return null;

  switch (args.action as string) {
    case "list": {
      const data = (await graphqlRequest(
        `query { invoices(company_id: "${args.company_id}") { ${INVOICE_FIELDS} } }`,
        undefined,
        true
      )) as { invoices: unknown[] };
    ...
    case "get": {
      const data = (await graphqlRequest(
        `query { invoice(id: "${args.id}") { ${INVOICE_FIELDS} } }`,
        undefined,
        true
      )) as { invoice: unknown };
    ...
    case "create": {
      ...
      if (args.customer_email) input.customer_email = args.customer_email;
      if (args.customer_address) input.customer_address = args.customer_address;
    ...
    case "update_status": {
      const data = (await graphqlRequest(
        ...
        true
      )) as { updateInvoiceStatus: unknown };
      return {
        content: [
