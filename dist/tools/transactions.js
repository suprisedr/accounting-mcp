...
export const tools = [
    {
        ...
        description: "Create a double-entry journal transaction. Requires prior login. Each transaction must balance (total debits == total credits). " +
        ...
        inputSchema: {
            ...
            properties: {
                ...
                source_document: {
                    type: "string",
                    description: "The source document this entry is taken from (e.g. 'bank statement', 'supplier invoice', 'sales receipt', 'petty cash voucher'). Required.",
                },
                notes: {
                    type: "string",
                    description: "Short explanatory note — MAXIMUM 10 WORDS — describing why the entry is posted (e.g. 'Fuel for delivery van paid by card'). Required.",
                },
                lines: {
                    type: "array",
                    description: "Journal lines. Must balance — total DEBIT amounts must equal total CREDIT amounts.",
                    items: {
                        type: "object",
                        properties: {
                            chart_of_account_id: {
    ...
    },
    {
        ...
        description: "Check whether a company is VAT-registered. Call this BEFORE creating any transaction to determine whether VAT journal lines are required. " +
    ...
    },
    {
        ...
        description: "Fetch chart of accounts for a given company. Returns accounts with nested child accounts (items) and optional parent info. Set only_parents to true to return only top-level parent accounts with their children. Requires prior login.",
        inputSchema: {
            type: "object",
            properties: {
                ...
                only_parents: {
                    type: "boolean",
                    description: "When true, returns only parent (top-level) accounts with their nested child accounts. Defaults to false (returns all accounts).",
                },
            },
            required: ["company_id"],
        },
    },
    {
        ...
        description: "Fetch a single posted transaction by its reference (e.g. JNL-0001) for a given company. " +
    ...
    },
    {
        ...
        description: "Semantic (vector) search over posted transactions. Use this when the user describes what they're looking for in natural language " +
    ...
    },
    {
        ...
        description: "Suggests industry-appropriate, IFRS-compliant chart of account entries to add when no suitable account exists for a transaction line. Call this before blocking a transaction — present the returned suggestions to the user and ask whether they want to add one or proceed with an existing account.",
        inputSchema: {
            type: "object",
            properties: {
                purpose: {
                ...
                industry: {
                    type: "string",
                    description: "The industry the business operates in (e.g. 'retail', 'construction', 'healthcare', 'hospitality', 'manufacturing', 'technology', 'services', 'agriculture').",
                },
                account_type: {
                    type: "string",
...
async function handleGetChartOfAccounts(args) {
    const company_id = args.company_id;
    const only_parents = args.only_parents;
  ...
  `;
    const data = (await graphqlRequest(query, undefined, true));
    return { content: [{ type: "text", text: JSON.stringify(data.chartOfAccounts, null, 2) }] };
}
async function handleGetVatRegistrationStatus(args) {
    const data = (await graphqlRequest(`query { vatRegistrationStatus(company_id: "${args.company_id}") { id vat_number registered_at deregistered_at is_active } }`, undefined, true));
    ...
    if (!status || !status.is_active) {
        return {
            content: [
                {
                    type: "text",
                    text: JSON.stringify({ vat_registered: false, message: "Company is NOT VAT-registered. Do not include any VAT journal lines." }, null, 2),
                },
            ],
        };
    }
        ...
        content: [
            {
                ...
                text: JSON.stringify({
                    ...
                    message: "Company IS VAT-registered. Include a VAT journal line: CR VAT Output Tax for sales/revenue, DR VAT Input Tax for expenses/purchases.",
                }, null, 2),
            },
        ],
    };
...
async function handleCreateTransaction(args) {
    const { company_id, transaction_date, description, reference, source_document, notes, lines } = args;
    if (!source_document || !source_document.trim()) {
        throw new Error("source_document is required. Specify the document this entry is taken from (e.g. 'bank statement', 'supplier invoice').");
    }
    if (!notes || !notes.trim()) {
    ...
    if (wordCount > 10) {
        throw new Error(`notes must be 10 words or fewer (got ${wordCount}). Shorten the explanatory note before posting.`);
    }
    ...
    const coaData = (await graphqlRequest(`query { chartOfAccounts(company_id: "${company_id}") { id account_code account_name items { id } } }`, undefined, true));
    ...
    if (offendingLines.length > 0) {
        const details = offendingLines
            ...
            .join("; ");
        throw new Error(`Cannot post to parent account(s): ${details}. Parent accounts are roll-up containers, not posting accounts. ` +
  ...
  `;
    const data = (await graphqlRequest(mutation, { input: { company_id, transaction_date, description, reference, source_document, notes, lines } }, true));
    return {
        content: [{ type: "text", text: JSON.stringify({ success: true, transaction: data.createTransaction }, null, 2) }],
    };
}
async function handleGetTransactionByReference(args) {
    const { company_id, reference } = args;
    const query = `
  ...
  `;
    const data = (await graphqlRequest(query, { companyId: company_id, reference }, true));
    if (!data.transactionByReference) {
        return {
            content: [
                {
                    type: "text",
                    text: JSON.stringify({ found: false, message: `No transaction found with reference "${reference}" for company ${company_id}.` }, null, 2),
                },
            ],
        };
    }
...
async function handleSearchTransactions(args) {
    const { prompt, limit, company_id } = args;
    const query = `
  ...
  `;
    const data = (await graphqlRequest(query, { prompt, limit: limit ?? 10, companyId: company_id ?? null }, true));
    ...
    if (results.length === 0) {
        return {
            content: [
                {
                    type: "text",
                    text: JSON.stringify({ found: false, message: `No transactions matched "${prompt}" with similarity >= 0.5.` }, null, 2),
                },
            ],
        };
    }
...
async function handleSearchDocuments(args) {
  ...
  `;
    const data = (await graphqlRequest(query, { p: prompt }, true));
    ...
    if (results.length === 0) {
        return {
            content: [
                {
                    type: "text",
                    text: JSON.stringify({ found: false, message: `No documents matched "${prompt}".` }, null, 2),
                },
            ],
        };
    }
