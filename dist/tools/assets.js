...
async function handleUpdateAsset(args) {
  ...
  `;
    const data = (await graphqlRequest(mutation, { id, input }, true));
    return {
...
async function handleDisposeAsset(args) {
  ...
  `;
    const data = (await graphqlRequest(mutation, { id, date: disposal_date, proceeds: disposal_proceeds ?? null }, true));
    return {
        content: [
            {
                type: "text",
