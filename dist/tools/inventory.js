...
export const tools = [
    {
        name: "inventory",
        description: "Manage inventory items. Use the 'action' field to select the operation. Requires prior login.\n" +
...
export async function handle(name, args) {
    ...
    switch (args.action) {
        case "list": {
            const onlyActiveArg = args.only_active !== undefined ? `, only_active: ${args.only_active}` : "";
            const data = (await graphqlRequest(`query { inventoryItems(company_id: "${args.company_id}"${onlyActiveArg}) { ${ITEM_FIELDS} } }`, undefined, true));
        ...
        case "create": {
            ...
            const data = (await graphqlRequest(`mutation CreateInventoryItem($input: CreateInventoryItemInput!) {
          createInventoryItem(input: $input) { ${ITEM_FIELDS} }
        }`, { input: inputFields }, true));
            return {
                content: [
        ...
        case "update": {
            ...
            const data = (await graphqlRequest(`mutation UpdateInventoryItem($id: ID!, $input: UpdateInventoryItemInput!) {
          updateInventoryItem(id: $id, input: $input) { ${ITEM_FIELDS} }
        }`, { id, input: inputFields }, true));
            return {
                content: [
        ...
        case "delete": {
            const data = (await graphqlRequest(`mutation DeleteInventoryItem($id: ID!) {
